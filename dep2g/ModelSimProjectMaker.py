...
class ModelSimProjectMaker( object ):
  ...
  def write( self , aTarget, aScriptVariables , aComponentPaths , aCommandList , aLibs, aMaps ):
    ...
    for src in reversed( aCommandList["src"] ):
      if src.Include:
        ...
        if lExt == ".xco":
          ...
          for d in ["","sim"]:
            lPathToIp = join(lIpPath,lName,d,lName+".vhd")
          ...
          # file = abspath( join( split( self.CommandLineArgs.output )[0] , "top/top.srcs/sources_1/ip" , lName, "sim" , lName + ".vhd" ) )
          print(file)
        else:
          file = src.FilePath
    ...
    lSimLibDirectory = abspath( join( split( self.CommandLineArgs.output )[0] , "top/top.sim/sim_1/behav/msim") )
    ...
    for l in lSimLibs:
      lModelsimIni.set("Library",l,join(lSimLibDirectory,l))
