...
class VivadoPresynthScriptGenerator:
    ...
    def write(self, aOutput, aSettings, aComponentPaths, aCommandList, aLibs):
        ...
        lScriptContents=f"""
            ...
            #  3) git rev-parse --short=7 HEAD      => 7-char SHA
            #  4) git diff --quiet HEAD             => Non-zero exit code if local changes not included in commit

            set SOURCES_ROOTDIR [file normalize [file join [get_property DIRECTORY [current_project]] "../../../../../src"]]
            puts "SOURCES_ROOTDIR=$SOURCES_ROOTDIR"
            set SOURCES_SUBDIRS {{{' '.join(self.repo_list)}}}
            ...
            foreach SOURCE_AREA_NAME $SOURCES_SUBDIRS {{
              ...
              set IS_GIT_REPO [exec sh -c {{git rev-parse --is-inside-work-tree 2> /dev/null || echo false}}]
              ...
              if {{$GIT_REF == {{HEAD}}}} {{
                if {{ [catch {{exec git describe --exact-match --tags}} GIT_REF] }} {{
