...
def test_no_assign(dep_parser):
    ...
    assert dep_parser._line_process_assignments(line_no_assignment) == line_no_assignment


# -----------------------------------------------------------------------------
def test_assign(dep_parser):
    assert dep_parser._line_process_assignments('@ a = "a_value"') == None
    assert dep_parser.settings['a'] == 'a_value'
    assert dep_parser._line_process_assignments('@ c = a') == None
    assert dep_parser.settings['c'] == 'a_value'
    assert dep_parser._line_process_assignments('@ d.e.f = 3.4') == None
    assert dep_parser.settings['d.e.f'] == 3.4
    assert dep_parser._line_process_assignments('@ hls.cflags = "-std=c++11"') == None
...
def test_assign_invalid_expr(dep_parser):
    with pytest.raises(DepAssignmentError):
        dep_parser._line_process_assignments('@ hello world')


# -----------------------------------------------------------------------------
def test_assign_no_parname(dep_parser):
    with pytest.raises(DepAssignmentError):
        dep_parser._line_process_assignments('@ = s')


# -----------------------------------------------------------------------------
def test_assign_no_value(dep_parser):
    with pytest.raises(DepAssignmentError):
        dep_parser._line_process_assignments('@ x =')


# -----------------------------------------------------------------------------
def test_assign_invalid_parname(dep_parser):
    with pytest.raises(DepAssignmentError):
        dep_parser._line_process_assignments('@ 2_not_good = 0')


# -----------------------------------------------------------------------------
def test_assign_invalid_value(dep_parser):

    dep_parser._line_process_assignments('@ a = print(3)')
